...
resource "null_resource" "local_sql_exec" {
  ...
  provisioner "local-exec" {
    command = "powershell -Command \"& 'C:\\Program Files\\MySQL\\MySQL Server 8.0\\bin\\mysql.exe' -h ${aws_db_instance.mysql_rds.address} -u admin -pranjit-1234 dev < 'D:\\terraform\\terrarepo\\rds\\init.sql'\""
  }
